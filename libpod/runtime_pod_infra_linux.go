...
func (r *Runtime) makeInfraContainer(ctx context.Context, p *Pod, imgName, rawImageName, imgID string, config *v1.ImageConfig) (*Container, error) {
	...
	if config != nil {
		...
		if p.config.UsePodPID && p.config.InfraContainer.Pid.NSMode != "host" {
			g.AddOrReplaceLinuxNamespace(string(spec.LinuxNamespaceType("pid")), p.config.InfraContainer.Pid.Value)
		} else if p.config.InfraContainer.Pid.NSMode == "host" {
